...
fn main() {
    ...
    // CHANGE CODE counter: Don't change any code above this line
    ...
    let mutex: Arc<Mutex<Vec<Vec<usize>>>> = Arc::new(Mutex::new(xs));
    
    let t0;
    let xs_0 = Arc::clone(&mutex);
    t0 = thread::spawn(move || map_data(&xs_0.lock().unwrap()[0]));

    let t1;
    let xs_1 = Arc::clone(&mutex);
    t1 = thread::spawn(move || map_data(&xs_1.lock().unwrap()[1]));

    let val0 = t0.join().unwrap();
    let val1 = t1.join().unwrap();
    
    intermediate_sums.push(val0);
    intermediate_sums.push(val1);
    ...
    let data = partition_data(num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&data);

    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    let mut threads_vec: Vec<thread::JoinHandle<usize>> = Vec::new();
    let mutex: Arc<Mutex<Vec<Vec<usize>>>> = Arc::new(Mutex::new(data));
    let mut intermediate_sums_vec: Vec<usize> = Vec::new();

    for i in 0..num_partitions{
        let data_clone = Arc::clone(&mutex);
        let thread = thread::spawn(move || map_data(&data_clone.lock().unwrap()[i]));
    ...
    for thread in threads_vec{
        ...
        intermediate_sums_vec.push(val);
    }

    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", intermediate_sums_vec);

    // 5. Calls reduce_data to process the intermediate sums
    let new_sum = reduce_data(&intermediate_sums_vec);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let leftovers = v.len() % num_partitions;
    let mut xs: Vec<Vec<usize>> = Vec::new();
    let mut counter = 0;
    ...
    for i in 0..num_partitions{
        ...
        // Add the first half of the integers in the input vector to x1
        for j in 0..(partition_size + add_one){
            x.push(v[counter + j]);
        }
        xs.push(x);
        counter = counter + partition_size + add_one;
    }
    xs
}
